...
android {
    ...
    sourceSets {
        // Reticulum BLE (btleplug droidplug, generated by copyReticulumBleJava) uses
        // BLE scan APIs that require API 23, so it is added only to the minSdk-23
        // flavors (play, other) — never to the minSdk-21 otherLegacy flavor, where
        // lint's fatal NewApi check would (correctly) reject the API-23 calls. RNode
        // over BLE is unavailable on the legacy (Android 5) flavor.
        val bleJava = reticulumBleJavaDir.get().asFile.path
        getByName("play") {
            java.directories.addAll(listOf("src/minApi23/java", bleJava))
            aidl.directories.add("src/minApi23/aidl")
        }
        getByName("other") {
            java.directories.addAll(listOf("src/minApi23/java", "src/github/java", bleJava))
            aidl.directories.add("src/minApi23/aidl")
        }
    ...
    compileOptions {
        ...
        // The bundled jni-utils classes (used by btleplug's BLE backend) reference
        // java.util.function.Function/BiFunction (API 24). Desugaring backports them
        // so BLE works on the minSdk-23 (Android 6) play/other flavors.
...
dependencies {
    // Backports java.util.function.* etc. for the bundled jni-utils/btleplug BLE
    // classes so they load on API 23 (see isCoreLibraryDesugaringEnabled above).
